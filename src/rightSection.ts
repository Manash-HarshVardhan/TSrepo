// Right section
let cardcontainer = document.querySelector('.cardcontainer') as HTMLDivElement;
let inputsearch = document.querySelector('.inputsearch') as HTMLInputElement;
let filtertask = document.querySelector('.filtertask') as HTMLSelectElement;
let sortOptions = document.querySelector('.sortoption') as HTMLSelectElement; 
...
function createCard(cardArray: Array<{ id: number, title: string, descr: string, date: string, 
    priority: string, finished: boolean,work:boolean,home:boolean,personal:boolean }>) {
    ...
    cardArray.forEach(field => {
        ...
        let selectpriorityTxt: string = field.priority;
...
inputsearch.addEventListener('input', () => {
    ...
    if (searchValue) {
        const filterItem = cardArray.filter(
            field =>
                field.title.toLowerCase().startsWith(searchValue) ||
